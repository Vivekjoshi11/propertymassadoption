...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (addresses.length === 0) {
      sendEvent('No addresses found in the CSV file!');
      return res.end();
    }

    sendEvent('Saving addresses to JSON...');
    const destinationDirectory = path.join(process.cwd(), 'inputFiles');
    const destinationFilePath = path.join(destinationDirectory, 'allAddress.json');
    if (!fs.existsSync(destinationDirectory)) {
      fs.mkdirSync(destinationDirectory, { recursive: true });
    ...
    await retryOperation(() => runScript('./src/scripts/1-getCoOrdinates.ts'), 3, 5000);
    sendEvent('Script 1 completed.');

    console.log("running 2nd script");
    sendEvent('Running 2-addPropertiesToDb.js...');
    await retryOperation(() => runScript('./src/scripts/2-addPropertiesToDb.ts'), 3, 5000);
    sendEvent('Script 2 completed.');

    console.log("running 3rd script");
    sendEvent('Running 3-mintTx.js...');
    await retryOperation(() => runScript('./src/scripts/3-mintTx.ts'), 3, 5000);
    sendEvent('Script 3 completed.');

    sendEvent('All scripts executed successfully!');
    res.end();
  } catch (error) {
    console.error('Error processing CSV:', error);
    logErrorToFile(error);
